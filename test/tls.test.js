...
test('serves HTTPS when TLS_CERT_PATH and TLS_KEY_PATH are set', async () => {
  ...
  const app = await new NotifyApi({ config, queue: testQueue(config), presence: testPresence(), templates, channels: [], version: '1.0.0', logger: silentLog }).build();
  ...
  try {
    ...
    assert.deepEqual(body, { status: 200, data: '{"status":"ok"}', tls: true });
  } finally {
    await app.close();
  }
});
